import { Database, PlaySquare, RadioTower, Settings, Trophy, Users } from 'lucide-react';
    ...
    <main className="min-h-screen bg-gs-black text-gs-white">
      <div className="grid grid-cols-[260px_1fr] min-h-screen">
        <aside className="border-r border-gs-line bg-[#0b0b0c] p-6">
          ...
          <p className="text-gs-soft text-xs tracking-[0.3em] uppercase mt-1">Football Stats 2.0.1</p>
            ...
            {nav.map((item) => (
              <button
                ...
                className={`w-full flex items-center gap-3 rounded-2xl px-4 py-3 text-left transition ${
                  page === item.id
                    ? 'bg-gs-orange text-black font-bold'
                    : 'text-zinc-300 hover:bg-gs-card2 hover:text-white'
                }`}
              >
                <span className={page === item.id ? 'text-black' : 'text-gs-orange'}>{item.icon}</span>
                {item.label}
              </button>
            ))}
          </nav>
        ...
        <section>
          <header className="h-16 border-b border-gs-line bg-[#111113] flex items-center justify-between px-6">
            <div>
              ...
              <p className="text-xs text-zinc-400">Teamverwaltung, zentrale SQLite-Datenbank und modernes UI</p>
            </div>
            <div className="rounded-full bg-gs-orange text-black px-4 py-2 font-bold">v2.0.1</div>
          </header>
